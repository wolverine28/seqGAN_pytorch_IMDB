...
def generate_samples(model, batch_size, generated_num, SOS):
    samples = []
    for _ in range(int(generated_num / batch_size)):
        sample = model.sample(batch_size, SEQ_LEN, SOS).cpu().data.numpy().tolist()
...
def discriminator_train_epoch(model, real_loader, fake_loader, criterion, optimizer):
    ...
    for i, (_, real_data, _) in enumerate(tqdm(real_loader, mininterval=2, desc=' - Training', leave=False)):
        try:
        ...
        real_data = Variable(real_data[:,1:])
